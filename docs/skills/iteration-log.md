...
- **宫斗本首册暂时搁置**：试玩后确认宫斗题材对剧本要求太高，难以通过短期迭代达到"爽感"体验。已完成的优化（结局分布、回响矩阵、选项反馈、弹窗修复）已 push 到 `feature/full-story-experience` 分支。
- **金手指/系统流新题材确定为下一步方向**：需要更低的故事创作成本、更直接的正反馈循环。

### 已记录的设计方向（待脑暴细化）

- **词条系统（已记录）**：肉鸽式词条抽取，不同级别不同概率，影响剧情分支
- **死后转生机制（已记录）**：死亡后可继续游戏，继承部分能力或主动选择金手指
- **番茄小说《我一个史莱姆吊打巨龙很合理吧》系统机制（已调研）**：
  - 核心：神级选择系统 → 遇到事件弹出 A/B/C → 选对给奖励（属性/技能/进化/资源）
  - 主角：人类御兽师 + F 级金色史莱姆（反常识变强）
  - 爽点：等级偏见打脸、种族鄙视链反转、常识颠覆、装怂爆发流
  - 与现有框架天然适配：系统选择 = 剧情选择 + 奖励，御兽进化 = stats 变化
